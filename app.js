...
app.use(session({ secret: 'dealstryker', cookie: { maxAge: 60000 }, resave: false, saveUninitialized: false }));
...
if(!isProduction) {
  ...
  //mongoose.connect('mongodb://localhost/dealstryker', { useNewUrlParser: true });
 mongoose.connect('mongodb+srv://DealStryker:4rDlX5UcVur9K0f2@dealstryker-ieho9.mongodb.net/test?retryWrites=true&w=majority', { useNewUrlParser: true });
...
app.get('*', (req, res) => {
  res.sendFile(path.resolve(__dirname, 'client', 'build', 'index.html'), { maxAge: cacheRefresh });
});
