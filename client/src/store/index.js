...
export default ({ api, history, authManager }) => {
  ...
  const enhancers = process.env.NODE_ENV === 'production' ?
      compose(applyMiddleware(sagaMiddleware, thunkMiddleware, routerMiddleware)) :
      composeWithDevTools(applyMiddleware(sagaMiddleware, thunkMiddleware, routerMiddleware));
