...
const sendMsg = (email, title, text) => {
  const SENDGRID_API_KEY = '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';
  const isProduction = process.env.NODE_ENV === 'production';
  const link = isProduction ? 'https://dealstryker.com' : 'http://localhost:3000';
...
module.exports = function(socket){
        ...
        Users.findOne({_id: userId}, (err, foundedUser) => {
            ...
            if (foundedUser) {
                ...
                socket.on("disconnect", () => {
                    const userIndexInArray = connectedUsers.findIndex((foundedUser) => foundedUser.id === connectedUser.id);
                ...
                Bid.find({
                    ...
                    await Promise.all(docs.map(async (request) => {
                        let offersArr = [...request.responses];
                        await Offer.find({
                            '_id': { $in: offersArr}
                        }, (err, offers) => {
                            if (connectedUser.role === 'dealer')
                                newReq.push({ ...request._doc, responses: offers.filter((offer) =>
                                        offer.dealerId === connectedUser.id) });
              ...
              await Bid.find({
                ...
                if (activeBids && activeBids.length < 3) {
                  ...
                  let offerRequest = new Bid({
                    ...
                    distance: offerRequestData.distance,
                  ...
                  offerRequest.save().then(() => {
                    ...
                    Users.find({role: 'dealer'}, (error, result) => {
                      ...
                      if (result && result.length) {
                        const fitDealers = result.filter(dealer =>
                            zipcodes.distance(dealer.zip, offerRequest.zip) < offerRequest.distance
                        ...
                        if (fitDealers && fitDealers.length) {
                          fitDealers.forEach((fitDealer) => {
                            ...
                            if (fitNotificationsType.indexOf(fitDealer.notificationsType) !== -1)
                              sendMsg(fitDealer.email, 'New offer request', 'You received new offer request.');
                            fitDealer.linkBid(offerRequest._id);
                            fitDealer.save().then(() => {
                              const connectedDealer = connectedUsers.filter((connectedUser, i) => {
                                if (connectedUser.id === fitDealer.id && connectedUsers[i]) {
                                  connectedUsers[i].bids = [...connectedUsers[i].bids, offerRequest._id.toString()];
                                }
                                return connectedUser.id === fitDealer.id;
                              });
                              if (connectedDealer && connectedDealer.length) connectedDealer.map((dealer) => app.ioLiveBids.to(dealer.socketId).emit(OFFER_REQUEST_CREATED, offerRequest));
                            });
                          });
                        } else {
    ...
    socket.on(OFFER_CREATED, (offerData, callback) => {
        Bid.findOne({_id: offerData.requestId}, (err, foundedBid) => {
            Offer.findOne({ parentBidId: foundedBid._id, dealerId: socket.user.id }, (err, foundedOffer) => {
              if (err) {
                console.log("Search in database error");
              }
              ...
              if (foundedOffer) {
                ...
                foundedOffer.save().then(() => {
                  if (foundedBid) {
                    Users.findOne({_id: foundedBid.userId}, (err, customer) => {
                      if (!err && customer) {
                        if (customer.unreadLiveBids.indexOf(foundedOffer._id) === -1) {
                          customer.markAsUnread(foundedOffer._id);
                          const fitNotificationsType = ['all', 'offer'];
                          if (fitNotificationsType.indexOf(customer.notificationsType) !== -1)
                            sendMsg(customer.email, 'Offer updated', `${foundedBid.manufacturer} ${foundedBid.car} offer from ${socket.user.name} was updated.`);
                          customer.save();
                        }
                      }
                    });
                  }
                  const connectedCustomer = connectedUsers.filter(connectedUser => connectedUser.id === foundedBid.userId);
                  if (connectedCustomer && connectedCustomer.length) connectedCustomer.map((customer) => app.ioLiveBids.to(customer.socketId).emit(
                      OFFER_UPDATED, offerData.requestId, foundedOffer._id, { price: offerData.price }));
                ...
                if (foundedBid) {
                  ...
                  offer.save().then(() => {
                    ...
                    foundedBid.save().then(() => Users.findOne({_id: offerData.dealerId}, (err, user) => {
                      if (err) {
                        console.log("Search in database error");
                      }
                      if (foundedBid)
                        if (user) {
                          ...
                          Users.findOne({_id: foundedBid.userId}, (err, customer) => {
                            if (!err && customer) {
                              ...
                              const fitNotificationsType = ['all', 'offer'];
                              if (fitNotificationsType.indexOf(customer.notificationsType) !== -1)
                                sendMsg(customer.email, 'New offer', 'You received new offer.');
                              customer.save();
                            }
                          });
                          ...
                          if (connectedCustomer && connectedCustomer.length) {
                            connectedCustomer.map((cuustomer) => app.ioLiveBids.to(cuustomer.socketId).emit(OFFER_CREATED, offer));
                          }
    ...
    socket.on(OFFER_ACCEPTED, (offerId, callback) => {
        Offer.findOne({ _id: offerId }, (err, foundedOffer) => {
            ...
            if (foundedOffer) {
                foundedOffer.accept();
                foundedOffer.save().then(() => {
                    const connectedDealer = connectedUsers.filter(connectedUser => connectedUser.id === foundedOffer.dealerId);
                    if (connectedDealer && connectedDealer.length) connectedDealer.map((dealer) => app.ioLiveBids.to(dealer.socketId).emit(OFFER_ACCEPTED, foundedOffer.parentBidId, foundedOffer._id));
                });
      ...
      Bid.findOne({ _id: bidId }, (err, foundedBid) => {
          ...
          if (foundedBid && foundedBid.userId === socket.user.id) {
              ...
              Offer.find({
                ...
                if (offers && offers.length) offers.map((offer) => {
              ...
              foundedBid.save().then(() => {
                ...
                const connectedDealer = connectedUsers.filter(connectedUser => connectedUser.bids.indexOf(bidId) !== -1);
                if (connectedDealer && connectedDealer.length) connectedDealer.map((dealer) => app.ioLiveBids.to(dealer.socketId).emit(END_CAMPAIGN, bidId));
              });
    ...
    socket.on(REQUEST_OUT_THE_DOOR_PRICE, (userId, offerId, callback) => {
        Offer.findOne({ _id: offerId }, (err, foundedOffer) => {
            ...
            if (foundedOffer) {
              const connectedDealer = connectedUsers.filter(connectedUser => connectedUser.id === foundedOffer.dealerId);
              if (connectedDealer && connectedDealer.length) connectedDealer.map((dealer) => app.ioLiveBids.to(dealer.socketId).emit(
                  REQUEST_OUT_THE_DOOR_PRICE, foundedOffer._id));
      ...
      Users.findOne({ _id: socket.user.id }, (err, foundedUser) => {
        if (foundedUser) {
          if (foundedUser.role === 'dealer') {
            Offer.findOne({ _id: offerId }, (err, foundedOffer) => {
              if (foundedOffer) {
                Bid.findOne({ _id: foundedOffer.parentBidId }, (err, foundedBid) => {
                  if (foundedBid) {
                    Users.findOne({ _id: foundedBid.userId }, (err, foundedReceiver) => {
                      if (foundedReceiver) {
                        const fitNotificationsType = ['all', 'chat'];
                        if (fitNotificationsType.indexOf(foundedReceiver.notificationsType) !== -1)
                          sendMsg(foundedReceiver.email, 'New message', 'Your received new message from dealer');
                      }
                    });
                  }
                });
            ...
            Offer.findOne({ _id: offerId }, (err, foundedOffer) => {
              if (foundedOffer) {
                Users.findOne({ _id: foundedOffer.dealerId }, (err, foundedReceiver) => {
                  if (foundedReceiver) {
                    const fitNotificationsType = ['all', 'chat'];
                    if (fitNotificationsType.indexOf(foundedReceiver.notificationsType) !== -1)
                      sendMsg(foundedReceiver.email, 'New message', 'Your received new message from customer');
                  }
                });
              }
            });
