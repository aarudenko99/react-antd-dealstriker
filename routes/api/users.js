...
router.post('/registerCustomer', auth.optional, (req, res, next) => {
  ...
  Users.findOne({ email: user.email }, (err, foundedUser) => {
    ...
    if (foundedUser) {
      ...
      const finalUser = new Users({...user, role: 'customer', createdAt: new Date().getTime(), notificationsType: 'all'});
...
router.post('/registerDealer', auth.optional, (req, res, next) => {
  ...
  Users.findOne({ email: user.email }, (err, foundedUser) => {
    ...
    if (foundedUser) {
      ...
      const finalUser = new Users({...user, role: 'dealer', createdAt: new Date().getTime(), notificationsType: 'all'});
...
router.post('/oauth/facebook', auth.optional, (req, res, next) => {
  return passport.authenticate('facebookToken', {session: false}, (err, passportUser, info) => {
    if (err) {
      return next(err);
    }
...
router.post('/oauth/google', auth.optional, (req, res, next) => {
  return passport.authenticate('googleToken', {session: false}, (err, passportUser, info) => {
    if (err) {
      return next(err);
    }
...
router.post('/changePassword', auth.required, (req, res, next) => {
  ...
  if (!email) return res.sendStatus(422).json({ errors: { email: 'is required' } });
...
router.post('/resetPasswordReq', auth.optional, (req, res, next) => {
  ...
  Users.findOne({ email }, function(err, user) {
    if (!user) {
      ...
      const token = jwt.sign({ id: user._id, email: user.email }, `${user.hash}-${user.createdAt}`);
      const link = `${isProduction ? 'https://www.dealstryker.com' : 'http://localhost:3000'}/resetPassword?id=${user._id}&token=${token}`;
...
router.post('/resetPassword', auth.optional, (req, res, next) => {
  const {body: { id, token, newPassword }} = req;

  if (!id) return res.sendStatus(422).json({ errors: { id: 'is required' } });
  if (!token) return res.sendStatus(422).json({ errors: { token: 'is required' } });
  if (!newPassword) return res.sendStatus(422).json({ errors: { newPassword: 'is required' } });
  ...
  Users.findOne({ _id: id }, function(err, user) {
    if (!user) {
      ...
      jwt.verify(token, secret, (err, decoded) => {
        if (!err && decoded && decoded.email === user.email && !passRegExp.test(newPassword)) {
          user.setPassword(newPassword);
          user.save().then(() => res.status(200).json({ code: 200 }));
        }
        else return res.status(200).json({ code: 404, errors: { token: 'is invalid' } });
      });
    }
  });
});
...
router.post('/upload', auth.required, (req, res, next) => {
  ...
  const uploadPath = require('../../app').uploadsPath + new Date().getTime().toString() + '_' + sampleFile.name;
